...
function default_1(appRouter) {
     ...
     */
    appRouter.post('/api/v1/admin/brands', async (req, res) => {
        try {
            ...
            const brand = await Product_1.Brand.create({ name, description, logoUrl, status: status || 'ACTIVE' });
     ...
     */
    appRouter.put('/api/v1/admin/brands/:id', async (req, res) => {
        try {
            if (!await (0, authPlugin_1.admin)(req, res, appRouter))
                return;
            const { name, description, logoUrl, status } = await appRouter.parseJsonBody(req);
            const brand = await Product_1.Brand.findOne({ id: req.params.id });
            if (brand) {
     ...
     */
    appRouter.delete('/api/v1/admin/brands/:id', async (req, res) => {
        try {
            if (!await (0, authPlugin_1.admin)(req, res, appRouter))
                return;
            const brand = await Product_1.Brand.findOne({ id: req.params.id });
            if (brand) {
                await Product_1.Brand.deleteOne({ _id: brand._id });
