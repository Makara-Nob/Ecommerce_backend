 ...
 */
appRouter.get('/api/v1/cart', async (req: IncomingMessage, res: ServerResponse) => {
    try {
 ...
 */
appRouter.post('/api/v1/cart/items', async (req: IncomingMessage, res: ServerResponse) => {
    try {
        const userId = await protect(req, res, appRouter);
        if (!userId) return;
        ...
        let cartItem = await CartItem.findOne({ cartId: cart.id, product: product.id });
 ...
 */
appRouter.put('/api/v1/cart/items/:id', async (req: IncomingMessage & { params?: any }, res: ServerResponse) => {
    try {
        const userId = await protect(req, res, appRouter);
        if (!userId) return;
        ...
        if (!cartItem) return appRouter.sendResponse(res, 404, { message: 'Cart item not found' });
        
        // Verify ownership
        const cart = await Cart.findOne({ id: cartItem.cartId, userId });
        if (!cart) return appRouter.sendResponse(res, 403, { message: 'Item does not belong to your cart' });
 ...
 *         description: Cart item not found
 */
appRouter.delete('/api/v1/cart/items/:id', async (req: IncomingMessage & { params?: any }, res: ServerResponse) => {
    try {
        const userId = await protect(req, res, appRouter);
        if (!userId) return;
